...
impl Day16 {
    ...
    fn execute_move(
        ...
        if coord.x < 0
            || coord.y < 0
            || coord.y >= self.field.len() as i64
            || coord.x >= self.field[0].len() as i64
        {
            return;
        }
        ...
        // execute the move:
        let floor = self.field[coord.y as usize][coord.x as usize];
        match floor {
            // . --> move forward
            '.' => match direction {
                Direction::UP => {
    ...
    fn calc_energy(&mut self, start_pos: Coord2d, start_dir: Direction) -> u64 {
        ...
        while move_q.len() > 0 {
            ...
            self.execute_move(move_to, direction, &mut move_q, &mut visited);
        }
...
impl Day for Day16 {
    ...
    fn solve1(&mut self) -> String {
        ...
        solution = self.calc_energy(start_pos, start_dir);
    ...
    fn solve2(&mut self) -> String {
        ...
        for (start_pos, start_dir) in start_positions {
            let res = self.calc_energy(start_pos, start_dir);
            if res > max_energy {
                max_energy = res;
            }
        }
